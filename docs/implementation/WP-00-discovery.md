...
| Gate / approval | G0 **Pending**. G1: spike **#1 Langfuse PASSED** (agent, read-only); **#2 Vercel / #3 Teams / #4 AAD drafted, PENDING Jor** (see `docs/implementation/G1-spikes.md`). Others Pending. |
...
| Next safe step | **Offline coding exhausted until gates open.** Jor: run G1 spikes #2–#4 (`docs/implementation/G1-spikes.md`) + decide Metric Contract §6 + G0 HR sign-off. Agent-runnable now if approved: spike #4 AAD (read-only Graph). Then WP-12 (job/auth/delivery) → WP-13 (shadow, G2) → People WP-20+ (G0). |
