...
| Next safe step | WP-12 slices 1-5 drafted + tested (70 tests). **Activation needs Jor**: (1) run spike #3 Teams delivery; (2) provision QStash → set `QSTASH_TOKEN`/signing keys + `INSIGHT_WORKER_URL` in Vercel env (⚠️ no PowerShell-pipe CRLF); (3) create "BOB Insight Admins" AAD group → `INSIGHT_ADMIN_GROUP_ID`; (4) create `insight-analysis` prompt in Langfuse (label production; fallback file already shipped); (5) finish slice 6 deliver per spike #3; (6) set `INSIGHT_ENABLED=1`. Then WP-13 shadow (G2). |
