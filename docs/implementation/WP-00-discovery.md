...
| Commands run | `tsc --noEmit` → PASS. `npm test` → **42/42 PASS** (added channel test). `insight:report 7` + `spike-langfuse 7` run live (read-only): 199 turns/61 users, completeness 93.5%, cost $5.98/7d. |
| Decisions | WP-10: aggregate from **traces**. WP-11: **redaction is deterministic code**; **analysis prompt NOT hardcoded** (lives in Langfuse, WP-12); code contract is `validateAnalysis` + graceful degradation. Spike-driven fixes: read `channel` from `metadata.channel` (Langfuse sorts tags); `fetchTraces` backs off on **429** (traces endpoint is rate-limited). |
| Open assumptions | Vercel duration/detached-work (spike #2), Teams file delivery (spike #3), AAD group read (spike #4) — all drafted, awaiting Jor. HR columns (G0). Metric Contract §6 decisions (one-shot def, completeness threshold, UNKNOWN split). |
| Risks / known limitations | UNKNOWN not split (injection vs no-knowledge) — needs a new trace tag. Async job 60s cap → likely QStash pending spike #2. Traces endpoint rate-limited → fetch `limit=100`. |
| Next safe step | **WP-13 shadow validation (G2)**: run `/insight 7d` vs `npm run insight:report 7` for the same window ≥2 rounds, reconcile discrepancies (confirm same tz/filters), before wider rollout. Also (housekeeping): enforce per-job token/sample caps + daily job limit + cost alert (spec §5) before heavy use. People Connector (WP-20+) still needs G0. |
| Activation gotchas (solved) | QStash must set `QSTASH_URL` to the account's region endpoint (`https://qstash-us-east-1.upstash.io`) matching the Vercel function region (iad1) — else "user not found in this region". QStash signature verify drops the url-claim check (fragile). `INSIGHT_ADMIN_GROUP_ID` gates admins via `checkMemberGroups`. `scripts/insight-clear.mjs` resets stuck job state. |
